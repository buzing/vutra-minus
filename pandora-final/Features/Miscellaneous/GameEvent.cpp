...
void GameEvent::FireGameEvent(IGameEvent* pEvent) {
	...
	case hash_32_fnv1a_const("round_start"): {
		...
		/* this crashes you for whatever reason.
		if (g_Vars.misc.autobuy_enabled) {
			if (g_Vars.misc.autobuy_armor)
				m_pEngine->ExecuteClientCmd(pLocal->m_iAccount() < 1000 ? "buy vest" : "buy vesthelm");
			if (g_Vars.misc.autobuy_defusekit)
				m_pEngine->ExecuteClientCmd("buy defuser");
			if (g_Vars.misc.autobuy_hegrenade)
				m_pEngine->ExecuteClientCmd("buy hegrenade");
			if (g_Vars.misc.autobuy_smokegreanade)
				m_pEngine->ExecuteClientCmd("buy smokegrenade");
			if (g_Vars.misc.autobuy_molotovgrenade)
				m_pEngine->ExecuteClientCmd("buy incgrenade");
		}
		*/
