...
void LagCompensation::AddLatencyToNetchan(INetChannel* netchan, float Latency)
{
	...
	for (auto& seq : m_pSequences)
	{
		if (m_pGlobalVars->realtime - seq.curtime >= Latency)
		{
...
{
	if (!m_pEngine->GetNetChannelInfo())
		return;

	if (m_pSequences.empty() || m_pEngine->GetNetChannelInfo()->m_nInSequenceNr > m_pSequences.front().sequencenr) {
		// store new stuff.
		m_pSequences.emplace_front(m_pGlobalVars->realtime, m_pEngine->GetNetChannelInfo()->m_nInReliableState, m_pEngine->GetNetChannelInfo()->m_nInSequenceNr);
	}

	// do not save too many of these.
	while (m_pSequences.size() > 2048)
		m_pSequences.pop_back();
}