...
bool Chams::HandlePlayerChams(C_CSPlayer* player, std::function<void()> oFn) {
	...
	if (player->EntIndex() == pLocal->EntIndex()) {
		if (g_Misc.m_bMeetsTransparencyConditions)
			m_pRenderView->SetBlend(0.45f);
		else if (g_Vars.esp.chams_teammates)
			OverrideMaterial(false, g_Vars.esp.chams_teammates_color);
	...
	if (player != pLocal && !(player->IsTeammate(pLocal))) {
		if (g_Vars.esp.chams_player_wall) {
			OverrideMaterial(true, g_Vars.esp.chams_player_wall_color);
			oFn();
		}
	}

	if (g_Vars.esp.chams_player && !(player->IsTeammate(pLocal))) {
		OverrideMaterial(false, g_Vars.esp.chams_player_color);
		oFn();
