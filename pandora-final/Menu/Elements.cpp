			...
			if (GUI::Form::BeginTab(2, XorStr("C")) || GUI::ctx->setup) {
				...
				GUI::Group::BeginGroup("Player ESP", Vector2D(50, 69));
				{
					...
					if (visuals) {
						...
						if (GUI::Controls::Checkbox("Health color override", &visuals->health_color_override));
						GUI::Controls::ColorPicker("Health Color", &visuals->health_color);
				...
				GUI::Group::BeginGroup(XorStr("Other ESP"), Vector2D(50, 40));
				{
					...
					if (GUI::Controls::Checkbox("Aspect Ratio", &g_Vars.esp.aspect_ratio))
						GUI::Controls::Slider("#Range", &g_Vars.esp.aspect_ratio_value, 0.0f, 2.0f, "%.2f", 0, true);
				}
				GUI::Group::EndGroup();
				GUI::Group::PopLastSize();
				...
				GUI::Group::BeginGroup(XorStr("Effects"), Vector2D(50, 60));
				{
					...
					GUI::Controls::Checkbox("Remove sleeves", &g_Vars.esp.remove_sleeves);
					GUI::Controls::Checkbox("Remove landing bob", &g_Vars.esp.remove_bob);
			...
			if (GUI::Form::BeginTab(3, XorStr("D")) || GUI::ctx->setup) {
				...
				if (g_Vars.misc.autobuy_enabled) {
					std::vector <MultiItem_t > vBuyItems{
						...
						{"Defuser", &g_Vars.misc.autobuy_defusekit},
						{"HE Grenade", &g_Vars.misc.autobuy_hegrenade},
						{"Smoke Grenade", &g_Vars.misc.autobuy_smokegreanade},
						{"Molotov/Incendiary", &g_Vars.misc.autobuy_molotovgrenade},
						{"Taser", &g_Vars.misc.autobuy_zeus},
					};

					GUI::Controls::MultiDropdown("Utility", vBuyItems);
				}
				*/
				GUI::Group::EndGroup();
				...
				GUI::Group::BeginGroup(XorStr("Presets"), Vector2D(50, 25));
				{
					...
					bool reinit = false;
					if (initialise_configs || (GetTickCount64() % 1000) == 0) {
